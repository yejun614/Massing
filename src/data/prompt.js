/**
 * The prompt handed to a language model so it can author `.arch.json`.
 *
 * This string is the single source of truth. `node build.js --skill` writes it
 * out to `.claude/skills/massing-diagram/SKILL.md`, and the toolbar copies it
 ...
 * the sake of variety -- so the prompt opens with the defaults that avoid all
 * four and spends its length on those rather than on the field list.
 *
 * The prompt quotes a whole JavaScript file, so this module is the reason the
 * bundler blanks template literals before it scans: read literally, the
 * validator below declares `PLANES` at the top level and imports `node:fs`,
 * and neither is true of this file.
 */

import { componentsByCategory, GROUP_KINDS } from './components.js';

/**
 * The component list is generated rather than written out. At 100+ types a
 * hand-maintained copy drifts from the registry within a release or two, and a
 * prompt that offers a type the loader does not know is worse than useless.
 */
function typeCatalogue() {
  return componentsByCategory()
    .filter((cat) => cat.items.length)
    .map((cat) => `${cat.label}: ${cat.items.map((i) => '`' + i.type + '`').join(' ')}`)
    .join('\n');
}

function zoneCatalogue() {
  return GROUP_KINDS.map((k) => '`' + k.kind + '`').join(' ');
}

export const PROMPT_TITLE = 'Massing diagram authoring';

export const PROMPT_SUMMARY =
  'Write and edit Massing `.arch.json` isometric architecture diagrams.';

export const LLM_PROMPT = `You write Massing diagrams: \`.arch.json\` files describing isometric
