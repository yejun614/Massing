...
`.github/workflows/desktop.yml` builds five targets on tagged pushes and
...
arm64 Linux builds on an arm64 runner rather than cross-compiling, which would
need an arm64 toolchain and arm64 WebKitGTK headers through multiarch. Those
runners are free on public repositories and billed on private ones.

There is no arm64 Windows build. Nothing prevents it; nobody has asked.
 That file is
